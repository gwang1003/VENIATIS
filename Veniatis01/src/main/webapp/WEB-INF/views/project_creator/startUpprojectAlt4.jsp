...
<script>
$(function () {
	  var bank_sel = '<c:out value="${accountBank}"/>';


$("#groupCate").val(bank_sel).attr("selected", "selected");
			...
			<div class="apply_cont apply_support">
				<div class="box_comm">
	                ...
	                <div class="sheet_info">
	                	...
	                	<!-- 개설자 정보 -->
	                	<div class="form_area">
		                    <form action="startUpFinishProjectUpdate.do"name="addForm" method="post" enctype="multipart/form-data" onsubmit="return fn_validateCheck();">
									...
									<ul>
										<li class="box_info">
											...
											<p class="txt_input">
												<label for="tfFundingStart" class="tf_comm">	
													<input type="text" id="investStartDate" name="pStartDate_str" value="${sDate}" class="tf_cont input_hold">
												</label>
											</p>
										...
										<li class="box_info">
											...
											<p class="txt_input">
												<label for="tfFundingEnd" class="tf_comm">
													...
													<input type="text" id="investEndDate" name="pEndDate_str" value="${eDate}" class="tf_cont input_hold">
												</label>
											</p>
										...
										<li class="box_info">
                                            ...
                                            <p class="txt_input">
                                                ...
                                                <input type="text" id="targetAmt" name="pTargetAmount" value="${project.pTargetAmount}"  class="tf_cont">
                                                </label>
                                            </p>
										...
										<li class="box_info box_radio">
                                            ...
                                            <p class="txt_input">
                                                <label for="deliveryAmt" class="tf_comm">
	                                                ...
	                                                <input type="text" id="deliveryAmt" name="pAccount_num" class="tf_cont" value="${accountNum}">
                                                </label>
                                            </p>
										</li>
									</ul>
									<br>
									<c:if test="${accountBank == 'KB국민은행'}">
										<c:set var="selected1" value="selected"/>									
									</c:if>
									<c:if test="${accountBank == '신한은행'}">
										<c:set var="selected2" value="selected"/>									
									</c:if>
									<c:if test="${accountBank == '우리은행'}">
										<c:set var="selected3" value="selected"/>									
									</c:if>
									<c:if test="${accountBank == '하나은행'}">
										<c:set var="selected4" value="selected"/>									
									</c:if>
									<c:if test="${accountBank == '농협은행'}">
										<c:set var="selected5" value="selected"/>									
									</c:if>
									<c:if test="${accountBank == '제일은행'}">
										<c:set var="selected6" value="selected"/>									
									</c:if>
									<c:if test="${accountBank == '외환은행'}">
										<c:set var="selected7" value="selected"/>									
									</c:if>
									<c:if test="${accountBank == '카카오뱅크'}">
										...
										<li class="box_info box_group">
											...
											<div class="box_select">
												<select class="select_sort" id="groupCate" name="pAccount_bank" title="은행">
													...
													<option value="KB국민은행" ${selected1}>KB국민은행</option>
													<option value="신한은행" ${selected2}>신한은행</option>													
													<option value="우리은행" ${selected3}>우리은행</option>													
													<option value="하나은행" ${selected4}>하나은행</option>													
													<option value="농협은행" ${selected5}>농협은행</option>													
													<option value="제일은행" ${selected6}>SC제일은행</option>													
													<option value="외환은행" ${selected7}>외환은행</option>													
													<option value="카카오뱅크" ${selected8}>카카오뱅크</option>
                    ...
                    <div class="btn_area">
                        	...
                        	<input type="submit" class="btn_enroll" title="프로젝트 접수" value="프로젝트 접수" id="btn">
